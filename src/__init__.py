...
class Game:
    ...
    def __init__(self, proc, input_, replay=False, keys=[]):
        """
        ...
        self.spells = []
        self.iterations = 0
        self.input_iteration_map = []

        # Start the game loop, running the game
        self.start_game_loop(proc, input_, replay, keys)
    ...
    def start_game_loop(self, proc, input_, replay, keys):
        """
        ...
        while True:
            ...
            if end_condition == "win":
                ...
                # Save input_iteration_map to file
                if not replay:
                    replay_array = json.dumps(self.input_iteration_map)
                    path = sys.path[0] + f"/replays/replay_{int(datetime.datetime.timestamp(datetime.datetime.now()))}.json"
                    with open(path, "w") as outfile:
                        outfile.write(replay_array)
            ...
            # -----------TAKE USER INPUT (OR PERFORM REPLAY)-----------#

            key_was_pressed = False
            if replay and replay_index != len(keys) and self.iterations == keys[replay_index]["iteration"]:
                key_was_pressed = True

            # Check for input
            if (not replay and input_.if_key_pressed()) or (replay and key_was_pressed):
                # Get the input
                if not replay:
                    key = input_.get_input().lower()
                else:
                    key = keys[replay_index]["key"]
                    replay_index += 1

                # Storing the input and iteration for replay
                if not replay:
                    self.input_iteration_map.append(
                        {"iteration": self.iterations, "key": key}
                    )
